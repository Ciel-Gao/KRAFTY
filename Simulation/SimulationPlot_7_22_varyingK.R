...
## Fixed sigma=0.1, true K varying. Panel (a) with Z matrix, panel (b) with U matrix [True K known and unknown]
## True K = 4, 9, 15, sigma varying. Top 3 panels with Z matrix, bottom 3 panels with U matrix. [True K known and unknown]
## Combined U/Z sigma =0.01 and 1, true K varying. [True K unknown]
## Combined U/Z true K = 4, 9, 15, sigma varying. Definitely make a comment about ARI increasing with sigma! [True K unknown]
## Combined U/Z, sigma=0.1, true K varying. [True K known]
## 4-heatmap of sigma vs. TrueK with U vs. Z and KR vs. MASE fixed
## try looking at the heatmap of sigma vs. TrueK where we have error for Z - error for U, with KR vs. MASE fixed; and the heatmaps of sigma vs. TrueK where we have error for MASE - error for KR, with U vs. Z fixed.

library(tidyverse)
library(gridExtra)

load("ExperimentResults7-22-25_varyingTrueK.Rdata")
...
Zplot <- construct_df(Zresults.known, trueKvec, sigmavec) # Z results, K known
Zkplot <- construct_df(Zkestresults, trueKvec, sigmavec) # Z results for estimating true K
Unknown_Zplot <- construct_df(Zresults.unknown, trueKvec, sigmavec) # Z results, K unknown
...
# Panel (a) with Z matrix, panel (b) with U matrix [True K known and unknown] 
...
p1 <- draw_plots(
    ...
    name = "Joint clusters estimation: varying K with sigma fixed, Z and U results",
    metric = "ARI") + ylim(0.2, 1)
plot(p1)
ggsave("New_Figures/varyingK/K_ZU.png", plot = p1, width = 10, height = 5, dpi = 300)

# plot 2: True K = 4, 9, 15, sigma varying. 
# Top 3 panels with Z matrix, bottom 3 panels with U matrix. [True K known and unknown]
...
p2 <- draw_plots(
    ...
    name = "Joint clusters estimation: varying sigma with K fixed, Z and U results",
    metric = "ARI") + ylim(0.2, 1)
plot(p2)
ggsave("New_Figures/varyingK/K_ZU_sigma.png", plot = p2, width = 12, height = 5, dpi = 300)
# plot 3: Combined U/Z sigma =0.01 and 1, true K varying. [True K unknown]
# keep records when k is unknown, sigma == 0.01 or 1
df_compareZU_K <- full_df %>% filter(sigma %in% c(0.01, 1), type == "unknownK") %>%
    mutate(group_id = paste0(method, ", ", matrix)) %>%
    mutate(group_id = factor(group_id, levels = c("KR, Z", "KR, U", "MASE, Z", "MASE, U")))


p3 <- draw_plots(
    df = df_compareZU_K,
    changing_variable = "trueK",
    name = "Joint clusters estimation: varying K, comparing Z and U results",
    metric = "ARI",
    combine = TRUE) + ylim(0.2, 1)
plot(p3)
ggsave("New_Figures/varyingK/K_compareZU_K.png", plot = p3, width = 10, height = 5, dpi = 300)
# plot 4: Combined U/Z true K = 4, 9, 15, sigma varying. [True K unknown]
# keep records when k is unknown
df_compareZU_sigma <- full_df %>% filter(trueK %in% c(4, 9, 15), type == "unknownK") %>%
    mutate(group_id = paste0(method, ", ", matrix)) %>%
    mutate(group_id = factor(group_id, levels = c("KR, Z", "KR, U", "MASE, Z", "MASE, U")))

p4 <- draw_plots(
    df = df_compareZU_sigma,
    changing_variable = "sigma",
    name = "Joint clusters estimation: varying sigma, comparing Z and U results",
    metric = "ARI",
    combine = TRUE) + ylim(0.2, 1)
plot(p4)
ggsave("New_Figures/varyingK/K_compareZU_sigma.png", plot = p4, width = 10, height = 5, dpi = 300)

# plot 5: Combined U/Z, sigma=0.1, true K varying. [True K known]
# keep records when k is known and sigma = 0.1
df_compareZU_knownK <- full_df %>% filter(sigma == 0.1, type == "knownK") %>%
    mutate(group_id = paste0(method, ", ", matrix)) %>%
    mutate(group_id = factor(group_id, levels = c("KR, Z", "KR, U", "MASE, Z", "MASE, U")))

p5 <- draw_plots(
    df = df_compareZU_knownK,
    changing_variable = "trueK",
    name = "Joint clusters estimation: varying true K, comparing Z and U results, K known, \nsigma=0.1",
    metric = "ARI",
    combine = TRUE) + ylim(0.7, 1)
plot(p5)

ggsave("New_Figures/varyingK/K_compareZU_knownK.png", plot = p5, width = 6, height = 5, dpi = 300)
...
p6 <- ggplot(ZU_diff, aes(x = factor(sigma), y = factor(trueK), fill = diff)) +
    geom_tile() +
...
p6
ggsave("New_Figures/varyingK/K_compareZU_estK.png", plot = p6, width = 9, height = 5, dpi = 300)
...
p7
ggsave("New_Figures/varyingK/K_compareMethod_estK.png", plot = p7, width = 9, height = 5, dpi = 300)



## check the width of the largest credible interval using Z and U in estimating K.
cat("Using Z:")
Zkplot %>% group_by(method) %>% dplyr::summarize(max_diff = max(upper - lower))

cat("Using U:")
Ukplot %>% group_by(method) %>% dplyr::summarize(max_diff = max(upper - lower))
