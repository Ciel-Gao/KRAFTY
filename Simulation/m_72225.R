  ...
  for(kidx in 1:num_K){
    ...
    for(sigmaidx in 1:num_sigma){
      ...
      trialresultsall <- foreach (i = 1:ntrials,.combine=rbind,.packages=c("MASS","MGLM","mclust")) %dopar% {
        ...
        svd1 <- svd(Y1)
        svd2 <- svd(Y2)
        U1 <- svd1$u[,1:min(m,k1)]
        U2 <- svd2$u[,1:min(m,k2)]
        ...
        known_results <- c(compareWithTruth(svd.Z.kr$u[,1:min(dim(Zkr)[2],trueK)],trueK,truelabels),
                           compareWithTruth(svd.Z.mase$u[,1:min(dim(Zmase)[2],trueK)],trueK,truelabels),
                           compareWithTruth(svd.U.kr$u[,1:min(dim(Ukr)[2],trueK)],trueK,truelabels),
      ...
      Zresults.known[midx,kidx,sigmaidx,] <- 
      ...
      Uresults.known[midx,kidx,sigmaidx,] <- 
      ...
      Zresults.unknown[midx,kidx,sigmaidx,] <- 
      ...
      Uresults.unknown[midx,kidx,sigmaidx,] <- 
...
save(k1,k2,mvec,trueKvec,sigmavec,ntrials,mult,Zresults.known,Uresults.known,
     Zresults.unknown,Uresults.unknown,Zkestresults,Ukestresults,
     file="ExperimentResults7-22-25_varyingm.RData")
