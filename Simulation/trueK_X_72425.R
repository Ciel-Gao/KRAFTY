# Running first: 7/24 at 12:22 PM
...
dimvec <- c(num_K,num_sigma)
nsetups <- num_K*num_sigma
...
for(kidx in 1:num_K){
  ...
  for(sigmaidx in 1:num_sigma){
    ...
    Xresults.unknown[kidx,sigmaidx,] <- 
...
save(k1,k2,m,trueKvec,sigmavec,ntrials,mult,Xresults.known,
     Xresults.unknown,Xkestresults,
     file="ExperimentResults7-24-25_X_varyingTrueK.RData")
