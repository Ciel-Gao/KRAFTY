    ...
    for(Kidx in 1:4){
      ...
      for(sigmaidx in 1:num_sigma){
        ...
        trialresultsall <- foreach (i = 1:ntrials,.combine=rbind,.packages=c("MASS","MGLM","mclust")) %dopar% {
          ...
          svd1 <- svd(Y1)
          svd2 <- svd(Y2)
          X1 <- svd1$u[,1:k1] %*% diag(sqrt(svd1$d[1:k1]))
          X2 <- svd2$u[,1:k2] %*% diag(sqrt(svd2$d[1:k2]))
          Xkr <- kr(X1,X2)
          Xmase <- cbind(X1,X2)
          svd.X.kr <- svd(Xkr)
          svd.X.mase <- svd(Xmase)
          #known cluster numbers
          known_results <- c(compareWithTruth(svd.X.kr$u[,1:trueK],trueK,truelabels),
        ...
        Xresults.known[k1idx,k2idx,Kidx,sigmaidx,] <- 
        ...
        Xresults.known[k2idx,k1idx,Kidx,sigmaidx,] <- Xresults.known[k1idx,k2idx,Kidx,sigmaidx,]
        Xresults.unknown[k1idx,k2idx,Kidx,sigmaidx,] <- 
        ...
        # Xresults.unknown[k2idx,k1idx,Kidx,sigmaidx,] <- Xresults.known[k1idx,k2idx,Kidx,sigmaidx,]
        Xresults.unknown[k2idx,k1idx,Kidx,sigmaidx,] <- Xresults.unknown[k1idx,k2idx,Kidx,sigmaidx,]
        Xkestresults[k1idx,k2idx,Kidx,sigmaidx,] <-
          c(max(c(means[5]-mult*sds[5],0)),means[5],means[5]+mult*sds[5],
            max(c(means[6]-mult*sds[6],0)),means[6],means[6]+mult*sds[6])
        Xkestresults[k2idx,k1idx,Kidx,sigmaidx,] <- Xkestresults[k1idx,k2idx,Kidx,sigmaidx,]
...
save(kvec,m,sigmavec,ntrials,mult,Xresults.known,
     Xresults.unknown,Xkestresults,
     file="ExperimentResults8-7-25_X_varyingk1k2.RData")
